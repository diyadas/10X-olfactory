...
setwd("C:/RC_sst")
#devtools::install_github('renozao/NMF@devel')  #bugfix for getName plotheatmap problem 3.7.18
#remove("NMF")
library("NMF")
...
#install.packages("clusterExperiment")
#install.packages("C:/RCOB/clusterExperiment-1.3.6.tar", repos = NULL, type="source")
#source("https://bioconductor.org/biocLite.R")
#biocLite("clusterExperiment", dependencies= TRUE)
library(devtools)
install_github("epurdom/clusterExperiment", dependencies = TRUE)   #1.99.2 March 27 18
library(clusterExperiment) #installed RCurl, digest, Rcpp, lazyeval, tibble, mvtnorm, xml2, tidyr; update22718 digest, rlang, XML, taxize (taxizedb)
packageVersion("clusterExperiment")
library(ggplot2)

setwd("C:/RC_sst")
load("clusterexperiment_all_batches.rda") #clusterexp object has normalized counts
head(rownames(cl2))
source("https://bioconductor.org/biocLite.R")
biocLite("scater")
library(scater)
...
library(Rtsne)
source("https://bioconductor.org/biocLite.R") #Mar718
biocLite("zinbwave",dependencies=TRUE)
library(zinbwave)
```
...
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue",
            ...
            clusterFeaturesData=unique(genes[,"IndexInOriginal"]), labCol=rep("",ncol(cl2)), breaks=.99)
...
tsne_sub <- Rtsne(cl2[-wh_rm,], pca = FALSE, max_iter=5000)
...
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", breaks=.95,
            sampleData = c("gfp", "Batch"),
            clusterFeaturesData=unique(genes_1vall[,"IndexInOriginal"]), labCol=rep("",ncol(cl2)))


# RKC cluster specificity one v. all
write.table(genes_1vall , sep="\t", quote=FALSE, file =      "C:/RC_sst/output/DE/sstx94_genes_1vall.txt")

s0_DE <- genes_1vall[genes_1vall$ContrastName=="Cl07",]
s0_DE <- s0_DE[order(s0_DE$logFC, decreasing=TRUE),]
length(s0_DE$Feature)==length(unique(s0_DE$Feature))
shortlist_s0_DE<-s0_DE[1:68,]

s1_DE <- genes_1vall[genes_1vall$ContrastName=="Cl02",]
s1_DE <- s1_DE[order(s1_DE$logFC, decreasing=TRUE),]
shortlist_s1_DE<-s1_DE[1:49,]

s2_DE <- genes_1vall[genes_1vall$ContrastName=="Cl01",]
s2_DE <- s2_DE[order(s2_DE$logFC, decreasing=TRUE),]
shortlist_s2_DE<-s2_DE[1:74,]

s7_DE <- genes_1vall[genes_1vall$ContrastName=="Cl11",]
s7_DE <- s7_DE[order(s7_DE$logFC, decreasing=TRUE),]
length(s7_DE$Feature)==length(unique(s7_DE$Feature))
shortlist_s7_DE<-s7_DE[1:74,]

lists7not2DE <- setdiff(s7_DE$Feature,s2_DE$Feature)
shortlists7not2<-lists7not2DE[1:74]

s3_DE <- genes_1vall[genes_1vall$ContrastName=="Cl05",]
s3_DE <- s3_DE[order(s3_DE$logFC, decreasing=TRUE),]
shortlist_s3DE<-s3_DE[1:80,]

s4_DE <- genes_1vall[genes_1vall$ContrastName=="Cl04",]
s4_DE <- s4_DE[order(s4_DE$logFC, decreasing=TRUE),]
shortlist_s4_DE<-s4_DE[1:57,]

s5_DE <- genes_1vall[genes_1vall$ContrastName=="Cl03",]
s5_DE <- s5_DE[order(s5_DE$logFC, decreasing=TRUE),]
shortlist_s5_DE<-s5_DE[1:45,]

s6_DE <- genes_1vall[genes_1vall$ContrastName=="Cl10",]
s6_DE <- s6_DE[order(s6_DE$logFC, decreasing=TRUE),]
shortlist_s6_DE<-s6_DE[1:62,]

s8_DE <- genes_1vall[genes_1vall$ContrastName=="Cl14",]
s8_DE <- s8_DE[order(s8_DE$logFC, decreasing=TRUE),]
shortlist_s8_DE<-s8_DE[1:59,]

s9_DE <- genes_1vall[genes_1vall$ContrastName=="Cl09",]
s9_DE <- s9_DE[order(s9_DE$logFC, decreasing=TRUE),]
shortlist_s9_DE<-s9_DE[1:63,]

s10_DE <- genes_1vall[genes_1vall$ContrastName=="Cl12",]
s10_DE <- s10_DE[order(s10_DE$logFC, decreasing=TRUE),]
shortlist_s10_DE<-s10_DE[1:67,]

s11_DE <- genes_1vall[genes_1vall$ContrastName=="Cl06",]
s11_DE <- s11_DE[order(s11_DE$logFC, decreasing=TRUE),]
shortlist_s11_DE<-s11_DE[1:60,]


#log2(sconeNorm[AOB_m7DE$Feature[1:50],]+1
#assay(cmp)[m1DE$Feature[1:50],]+1
library("dendextend")
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", breaks=.95,
            sampleData = c("gfp", "Batch"),
            clusterFeaturesData=unique(shortlist_s8_DE[,"IndexInOriginal"]),  
            main=paste("sstx94_heatmap_1vall_zinb_all_batch_s8_DE"), 
            labCol=rep("",ncol(cl2)))

#devtools::install_github('renozao/NMF@devel')
NMF::nmf.options(grid.patch=TRUE)
pdf("output/viz/s10_DE_sstx94_heatmap_1vall_zinb_all_batch.pdf", height=8.5, width=11)
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", breaks=.95,
            sampleData = c("gfp", "Batch"),
            clusterFeaturesData=unique(shortlist_s10_DE[,"IndexInOriginal"]),  
            main=paste("sstx94_heatmap_1vall_zinb_all_batch_s10_DE"), 
            labCol=rep("",ncol(cl2)))
dev.off()

#inspect all clusters' resulting candidates in one heatmap w guiding controls
list1k<- c("Sst", "Pvalb", "Gfp", "tdTomato", "Slc17a7", "Rpb4", "Th", "Ecel1", "Anxa2", "Pld5", "Chrna2", "Sema3c", "Crh", "Calb2", "Sfrp2", "Calb1", "Mrap2", "Mme", "Hpse", "Akr1c18", "Rspo2", "Spp1", "Tacr1", "Ntn1",  "Myh8", "Baiap3", "Pgk1", "Lgals", "Klhl14", "Syndig1l", "Mobp", "Timp3", "Pdyn")
ulist1k <- intersect(list1k,rownames(cl2))

selectlist1k <- c("Crh", "Tacr1", "Calb2", "Hpse", "Sst", "Timp3", "Pld5", "Calb1" )
uselectlist1k <- intersect(selectlist1k,rownames(cl2))
#logSconeNorm <- (log2(assay(cl2)[ulist1k,] +1))
#hits_DE <- genes_1vall[genes_1vall[rownames(cl2) %in% logSconeNorm]]

NMF::nmf.options(grid.patch=TRUE)
pdf("output/viz/sstx94_heatmap_1vall_picksforISH.pdf", height=8.5, width=11)
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", 
            clusterFeaturesData=which(rownames(cl2) %in% uselectlist1k), sampleData = c("gfp", "Batch"),
            breaks=.99, main=paste("sstx94_heatmap_1vall_picksforISH"), labCol=rep("",ncol(cl2)))
dev.off()

NMF::nmf.options(grid.patch=TRUE)
pdf("output/viz/sstx94_heatmap_1vall_toppicksforISH.pdf", height=8.5, width=11)
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", 
            clusterFeaturesData=which(rownames(cl2) %in% uselectlist1k), sampleData = c("gfp", "Batch"),
            breaks=.99, main=paste("sstx94_heatmap_1vall_toppicksforISH"), labCol=rep("",ncol(cl2)))
dev.off()


#RKC best cluster classifiers pairs
pairsPerC <- getBestFeatures(cl2, contrastType="Pairs", number=100, p.value=0.05, isCount=FALSE)
#plotContrastHeatmap(cl2,pairsPerC) #use if 5 genes

write.table(pairsPerC , sep="\t", quote=FALSE, file =      "C:/RC_sst/output/DE/sstx94_genes_pairsPerC.txt")

#looking at known markers to inspect clusterID

NMF::nmf.options(grid.patch=TRUE)
pdf("output/viz/sstx94_heatmap_glutamatergicmarkers.pdf", height=8.5, width=11)
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", 
            clusterFeaturesData=which(rownames(cl2) %in% glutamatergic), sampleData = c("gfp", "Batch"),
            breaks=.99, main=paste("sstx94_heatmap_glutamatergicmarkers"), labCol=rep("",ncol(cl2)))
dev.off()

NMF::nmf.options(grid.patch=TRUE)
pdf("output/viz/sstx94_heatmap_gabaergicmarkers.pdf", height=8.5, width=11)
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", 
            clusterFeaturesData=which(rownames(cl2) %in% gabaergic), sampleData = c("gfp", "Batch"),
            breaks=.99, main=paste("sstx94_heatmap_gabaergicmarkers"), labCol=rep("",ncol(cl2)))
dev.off()

NMF::nmf.options(grid.patch=TRUE)
pdf("output/viz/sstx94_heatmap_nonneuronal.pdf", height=8.5, width=11)
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue", 
            clusterFeaturesData=which(rownames(cl2) %in% nonneruonal), sampleData = c("gfp", "Batch"),
            breaks=.99, main=paste("sstx94_heatmap_nonneuronal"), labCol=rep("",ncol(cl2)))
dev.off() 

##pairs plots to test candidate binary classifiers
anomalous <- c("Slc17a7", "Sst")
anomNorm<- which(rownames(cl2) %in% anomalous)
logSconeNorm <- (log2(assay(cl2)[anomalous,] +1))
pairs(genes_1vall[logSconeNorm,], pch=19, cex=0.5)

select7 <- c("Rspo2", "Spp1", "Pcdh8", "Timp3" ) #cl07 mediocre binary, first 2 are top picks
logSconeNorm <- (log2(assay(cl2)[select7,] +1))
pairs((genes_1vall[select7,]), pch=19, cex=0.5)
#make pdf of same
dev.copy(pdf,'output/viz/pairs_OBPN_classifiers_Clusm8.pdf', height=8.5, width=11)
dev.off ();
...
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue",
            sampleData = c("gfp", "Batch"),
            clusterFeaturesData=unique(genes_5[,"IndexInOriginal"]), breaks=.99, labCol=rep("",ncol(cl2)))
...
plotHeatmap(cl2, clusterSamplesData = "dendrogramValue",
            sampleData = c("gfp", "Batch"),
            clusterFeaturesData=which(rownames(cl2) %in% markers), breaks=.99, labCol=rep("",ncol(cl2)))
dev.off()
