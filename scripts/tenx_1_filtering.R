...
# Last revised: # Mon Jun 11 18:24:18 2018 ------------------------------
...
option_list <- list(
  ...
  make_option("--hkfile", default="../ref/hkl615.txt", type=character, help="path to file containing housekeeping genes"),
  make_option("--posctrlfile", default="../ref/oeRegPosCon.txt", type=character, help="path to file containing positive control genes")
)
...
pdf(file=file.path(vizdir, paste0(exptstr, "_QCindex_prefilt.pdf")), length=11, width=8.5)
...
for (feature in c("log10_total_counts", "ribo_pct", "mito_pct", "log10_total_features")){
...
# Filtering
colData(se) <- cbind(colData(se), qc) # store QC metrics with SummarizedExperiment object
...
mfilt <- metric_sample_filter(assay(se),
                              ...
                              plot = TRUE)
pdf(file=file.path(viz_dir, paste0(exptstr, "_msf_prefilt.pdf")), title = "metric_sample_filtering")
mfilt <- metric_sample_filter(assay(se),
                              nreads = colData(sce)$total_counts,
                              gene_filter = is_common,
                              pos_controls = hk_idx,
...
pcapairs_msf <- function(pca, mfilt, metric, title){
  plot(pca$x, pch=19, col=pal[as.numeric(mfilt[[metric]])+1],
...
pdf(file=file.path(viz_dir, paste0(exptstr, "_batchchk_prefilt.pdf")))
...
poscon <- intersect(as.character(read.table(opt$posctrlfile)$V1), rowData(filtered)$Symbol)
