...
option_list <- list(
  ...
  make_option("--seures", type = "character", default = "res0.5",
...
if (opt$clusmethod == "rsec"){
  ...
  colData(cl) <- DataFrame(cl@clusterMatrix, expt = colData(se_filtered)$expt,
...
clusterLegend <- setNames(rep(list(bigPalette), ncol(colData)), colnames(colData))
